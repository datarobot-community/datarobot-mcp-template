...
When enabled, the server will scan for deployments tagged as tools (tag: tool) and register each as a tool automatically. Please refer to the [dedicated guide on dynamic tool registration](.internal/dr-mcp-server-lib/base/docs/dynamic_tool_registration.md) to read more about this feature, supported workflows, and all the prerequisites.
