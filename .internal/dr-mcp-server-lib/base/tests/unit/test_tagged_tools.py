...
from base.core.mcp_instance import TaggedFastMCP, mcp
from base.core.telemetry import (
    _set_otel_attributes,
    get_trace_id,
    initialize_telemetry,
)
from base.core.tool_filter import (
    filter_tools_by_tags,
    get_tool_tags,
    get_tools_by_tag,
    list_all_tags,
)
from base.tools.core.mcp_server_tools import (
...
def test_get_all_available_tags_tool() -> None:
    ...
    with patch("base.tools.core.mcp_server_tools.mcp.get_all_tags") as mock_get_tags:
        # Test with tags
...
def test_dr_mcp_server_run_without_credentials() -> None:
    ...
    with patch(
        "base.core.dr_mcp_server.get_credentials", return_value=mock_credentials
    ):
        server = DataRobotMCPServer(mcp)
...
def test_telemetry_initialization() -> None:
    ...
    with patch("base.core.telemetry.get_config") as mock_config:
    ...
    with patch("base.core.telemetry.get_config") as mock_config:
        ...
        with patch("base.core.telemetry._setup_otel_env_variables"):
            with patch("base.core.telemetry._setup_otel_exporter"):
                with patch("base.core.telemetry._setup_http_instrumentors"):
                    with patch("base.core.telemetry.trace.get_tracer") as mock_tracer:
                        mock_span = Mock()
...
async def test_list_tools_by_tags_no_tags() -> None:
    ...
    with patch("base.tools.core.mcp_server_tools.mcp") as mock_mcp:
        # Mock tools
        mock_tool1 = Mock()
