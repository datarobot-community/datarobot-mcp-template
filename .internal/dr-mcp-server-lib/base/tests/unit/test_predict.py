...
from base.core.common import MCPError
from base.tools import predict


@pytest.fixture()
def patch_predict_dependencies() -> Generator[dict[str, Any], None, None]:
    with (
        patch("base.tools.predict.get_or_create_s3_credential") as mock_cred,
        patch(
            "base.tools.predict.make_output_settings",
        ...
        patch("base.tools.predict.dr.BatchPredictionJob") as mock_batch_job,
        patch(
            "base.tools.predict.generate_presigned_url",
...
async def test_predict_by_ai_catalog(
    ...
    with patch("base.tools.predict.uuid.uuid4", return_value="uuid"):
        mock_job = MagicMock()
        mock_job.id = "jobid"
        mock_job.status = "COMPLETED"
        patch_predict_dependencies["mock_batch_job"].score.return_value = mock_job
...
async def test_predict_by_file_path_timeout(
    ...
    assert (
        "Error in predict_by_file_path: AsyncTimeoutError: Job did not complete within the timeout period."
        == str(exc_info.value)
    )
...
async def test_predict_by_file_path_failure_error(
    ...
    with pytest.raises(MCPError) as exc_info:
        await predict.predict_by_file_path("dep", "file.csv", 1)
    assert (
        "Error in predict_by_file_path: AsyncFailureError: Job failed for some reason."
        == str(exc_info.value)
    )
