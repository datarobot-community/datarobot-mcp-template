...
from base.tools import project


@pytest.mark.asyncio
async def test_list_projects_success() -> None:
    with patch("base.tools.project.get_sdk_client") as mock_get_client:
...
async def test_list_projects_error() -> None:
    with patch("base.tools.project.get_sdk_client", side_effect=Exception("fail")):
