...
from base.core import credentials
...
def isolate_credentials(monkeypatch):
    ...
    monkeypatch.setattr(
        credentials.DataRobotCredentials, "model_config", config_without_env
    )
    monkeypatch.setattr(
        credentials.MCPServerCredentials, "model_config", config_without_env
    )
