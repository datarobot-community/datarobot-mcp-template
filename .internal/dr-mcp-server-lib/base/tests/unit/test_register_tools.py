...
def mock_mcp() -> Generator[MagicMock, None, None]:
    ...
    with patch("base.core.mcp_instance.mcp") as mock_mcp:
...
@pytest.mark.asyncio
@patch("base.core.mcp_instance.MemoryManager")
@patch("base.core.mcp_instance.get_memory_manager")
...
class TestFormatResponseAsToolResult:
    ...
    def test_format_response_by_content_type(
        self, test_case: FormatResponseTestCase
    ) -> None:
        ...
        if test_case.expected_data_key == "data":
            assert result.structured_content["data"] == test_case.data.decode(
                test_case.charset
            )
    ...
    def test_format_response_with_charset(self, test_case: CharsetTestCase) -> None:
        ...
        result = format_response_as_tool_result(
            test_case.data, "text/plain", test_case.charset
        )
