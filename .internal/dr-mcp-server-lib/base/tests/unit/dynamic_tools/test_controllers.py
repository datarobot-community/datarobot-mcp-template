...
from base.core.dynamic_tools.deployment.controllers import (
...
async def mcp_server():
    ...
    with (
        patch("base.core.dynamic_tools.deployment.controllers.mcp", test_mcp),
        patch("base.core.mcp_instance.mcp", test_mcp),
    ):
        yield test_mcp
...
def mock_external_dependencies():
    ...
    with (
        patch(
            "base.core.dynamic_tools.deployment.controllers.get_sdk_client"
        ) as mock_sdk,
        patch(
            "base.core.dynamic_tools.deployment.register.create_deployment_tool_config"
        ) as mock_config,
    ):
...
class TestDeploymentDeletion:
    ...
    async def test_delete_existing_deployment_with_logging(
        ...
        with patch(
            "base.core.dynamic_tools.deployment.controllers.logger"
        ) as mock_logger:
