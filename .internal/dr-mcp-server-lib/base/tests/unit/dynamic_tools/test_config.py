...
def _setup_mocks(monkeypatch, mock_metadata, mock_api_client):
    """Setup common mocks for all tests automatically."""
    monkeypatch.setattr(
        "base.core.dynamic_tools.deployment.config.get_mcp_tool_metadata",
        lambda *args, **kwargs: mock_metadata,
    )
    monkeypatch.setattr(
        "base.core.dynamic_tools.deployment.config.get_api_client",
...
class TestCreateDeploymentToolConfig:
    ...
    def test_create_config_with_serverless_deployment(
        ...
        mock_deployment.serverless_prediction_server = {
            "url": "https://serverless.example.com"
        }
    ...
    def test_create_config_strips_leading_slash_from_endpoint(
