...
from base.core.dynamic_tools import register
...
class TestGetOutboundHeaders:
    ...
    async def test_all_request_forwarded_headers_lowercase(self):
        ...
        assert configured_headers == configured_headers_lowercased, (
            "REQUEST_FORWARDED_HEADERS must be all lowercase, "
    ...
    @pytest.mark.asyncio
    async def test_empty_incoming_uses_spec_only(
        self, mock_get_http_headers, spec_model
    ):
    ...
    @pytest.mark.asyncio
    async def test_merge_precedence_when_keys_match_casing(
        self, mock_get_http_headers, spec_model
    ):
