...
from base.tools import data


@pytest.mark.asyncio
async def test_upload_dataset_to_ai_catalog_success() -> None:
    with (
        patch("base.tools.data.get_sdk_client") as mock_get_client,
...
async def test_upload_dataset_to_ai_catalog_file_not_found() -> None:
    with (
        patch("base.tools.data.get_sdk_client"),
...
async def test_upload_dataset_to_ai_catalog_error() -> None:
    with patch("base.tools.data.get_sdk_client", side_effect=Exception("fail")):
...
async def test_list_ai_catalog_items_success() -> None:
    with patch("base.tools.data.get_sdk_client") as mock_get_client:
        mock_client = MagicMock()
        mock_ds1 = MagicMock()
        mock_ds1.id = "1"
        mock_ds1.name = "ds1"
