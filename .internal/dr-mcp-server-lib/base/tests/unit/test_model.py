...
from base.tools import model


@pytest.mark.asyncio
async def test_get_best_model_success() -> None:
    with patch("base.tools.model.get_sdk_client") as mock_get_client:
...
async def test_get_best_model_error() -> None:
    with patch("base.tools.model.get_sdk_client", side_effect=Exception("fail")):
        with pytest.raises(Exception) as exc_info:
            await model.get_best_model("pid", "AUC")
