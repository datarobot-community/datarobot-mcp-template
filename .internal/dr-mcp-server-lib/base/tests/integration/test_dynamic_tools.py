...
from base.core.common import get_sdk_client
from base.core.credentials import get_credentials
from base.core.dynamic_tools.deployment.metadata import get_mcp_tool_metadata
from base.core.dynamic_tools.deployment.register import (
    register_tools_of_datarobot_deployments,
)
from base.core.mcp_instance import TaggedFastMCP
...
@pytest.fixture
def deployment_api_response_error(
    deployment_api_response_ok, deployment_id_error
) -> dict:
    ...
    deployment_response["description"] = (
        "An invalid tool deployment, without input schema."
    )
                ...
                "properties": {
                    "json": {
                        "properties": {
                            ...
                            "foo": {
                                "description": "The foo parameter controls how many Foo instances are needed.",
                                "title": "Foo parameter",
                                "type": "integer",
                            },
                            "otherParam": {
...
# todo - change the hardcoded deployment id (deployment_id_ok) to a proper fixture that creates a deployment in the test environment
@pytest.mark.skip(
    reason="Skipping test_get_mcp_tool_metadata until the deployment id is changed"
)
@responses.activate
...
async def test_dynamic_tool_registration(sdk_client, mock_api_responses) -> None:
    from base.core.mcp_instance import mcp
    ...
    assert "dynamic_tool_error" not in tool_names, (
        "`dynamic_tool_error` should not be registered."
    )
