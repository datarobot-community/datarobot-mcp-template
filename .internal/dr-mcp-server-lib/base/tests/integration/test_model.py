...
class TestMCPToolsIntegration:
    ...
    async def test_model_tools(self, classification_project: dict[str, Any]) -> None:
        ...
        async with integration_test_mcp_session() as session:
            ...
            assert (
                """Error calling tool 'get_best_model': Error in get_best_model: ClientError: 404 client error: {'message': 'Not Found'}"""
            ...
            assert (
                "Error calling tool 'score_dataset_with_model': Error in score_dataset_with_model: ClientError: 404 client error: {'message': 'Not Found'}"
                in result_text
            ), f"Result text: {result_text}"
