...
from .mcp_utils import ete_test_mcp_session
...
class TestPredictE2E(ToolBaseE2E):
    ...
    async def test_predict_by_file_path_success(
        ...
        async with ete_test_mcp_session() as session:
            ...
            test_name = (
                frame.f_code.co_name if frame else "test_predict_by_file_path_success"
            )
    ...
    async def test_predict_by_ai_catalog_success(
        ...
        async with ete_test_mcp_session() as session:
            frame = inspect.currentframe()
            test_name = (
                frame.f_code.co_name if frame else "test_predict_by_ai_catalog_success"
            )
            await self._run_test_with_expectations(
                prompt,
    ...
    async def test_predict_from_project_data_success(
        ...
        async with ete_test_mcp_session() as session:
            ...
            test_name = (
                ...
                if frame
                else "test_predict_from_project_data_success"
            )
            await self._run_test_with_expectations(
                prompt,
    ...
    @pytest.mark.skip(
        reason="Prediction explanations will be addressed in a different PR"
    )
    @pytest.mark.parametrize(
        "prompt_template",
        [
    ...
    async def test_get_prediction_explanations_success(
        ...
        async with ete_test_mcp_session() as session:
            ...
            test_name = (
                frame.f_code.co_name
                if frame
                else "test_get_prediction_explanations_success"
            )
            await self._run_test_with_expectations(
                prompt,
