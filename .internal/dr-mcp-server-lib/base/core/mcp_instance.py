...
class TaggedFastMCP(FastMCP):
    """
    Extended FastMCP that supports tags, deployments and other annotations directly in the tool decorator.
    """
    ...
    def tool(
        ...
        **kwargs: Any,
        ...
        """
        Extended tool decorator that supports tags and other annotations, while remaining
        ...
        if tags is not None:
            ...
            if annotations is None:
                annotations = ToolAnnotations(tags=tags_)  # type: ignore[attr-defined, unused-ignore]
            else:
    ...
    async def set_deployment_mapping(self, deployment_id: str, tool_name: str) -> None:
        """
        Adds or updates the mapping of a deployment ID to a tool name.
        ...
        if existing and existing != tool_name:
            logger.debug(
                f"Deployment ID {deployment_id} already mapped to {existing}, updating to {tool_name}"
            )
            try:
    ...
    async def remove_deployment_mapping(self, deployment_id: str) -> None:
        """
        Removes the mapping of a deployment ID to a tool name.
        ...
        if removed is not None:
            logger.debug(
                f"Removed deployment mapping for ID {deployment_id} with tool {removed}"
            )
            try:
...
async def register_tools(
    ...
    annotations_params = {"tags": tags, "deployment_id": deployment_id}
    annotations = ToolAnnotations(
        **{k: v for k, v in annotations_params.items() if v is not None}
    )
