...
from opentelemetry.trace import Span, SpanContext, Status, StatusCode, format_trace_id
...
class OpenTelemetryMiddleware(Middleware):
    ...
    async def on_request(
        self, context: MiddlewareContext, call_next: CallNext[Any, Any]
    ) -> Any:
        ...
        with self.tracer.start_as_current_span(f"tool.{tool_name}") as span:
            ...
            try:
                ...
                if hasattr(result, "content"):
                    span.set_attribute(
                        "mcp.tool.content_length", len(str(result.content))
                    )
...
def _setup_otel_env_variables() -> None:
    ...
    otlp_headers = f"X-DataRobot-Api-Key={credentials.datarobot.application_api_token},X-DataRobot-Entity-Id={entity_id}"
