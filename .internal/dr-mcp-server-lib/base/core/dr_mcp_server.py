...
from .telemetry import OtelASGIMiddleware, initialize_telemetry
...
class BaseServerLifecycle:
    ...
    async def post_server_start(self, mcp: FastMCP) -> None:
        """
        Called after the server has started and is ready to handle requests.
...
class DataRobotMCPServer:
    ...
    def __init__(
        ...
        lifecycle: Optional[BaseServerLifecycle] = None,
        additional_module_paths: Optional[list[tuple[str, str]]] = None,
    ):
        """
        ...
        Args:
            ...
            additional_module_paths: Optional list of (directory, package_prefix) tuples for loading additional modules
        """
        ...
        _import_modules_from_dir(
            os.path.join(base_dir, "tools", "core"), "base.tools.core"
        )
        if self._config.enable_predictive_tools:
            _import_modules_from_dir(os.path.join(base_dir, "tools"), "base.tools")

        _import_modules_from_dir(os.path.join(base_dir, "prompts"), "base.prompts")
        _import_modules_from_dir(os.path.join(base_dir, "resources"), "base.resources")

        # Load memory management tools if available
        if self._memory_manager:
            _import_modules_from_dir(
                os.path.join(base_dir, "tools", "core", "memory_management"),
                "base.tools.core.memory_management",
            )
...
def create_mcp_server(
    ...
    """
    Factory function to create a DataRobot MCP server.
