...
from base.core.common import get_api_client
from base.core.dynamic_tools.deployment.adapters.base import MetadataBase
from base.core.dynamic_tools.deployment.metadata import get_mcp_tool_metadata
from base.core.dynamic_tools.register import ExternalToolRegistrationConfig
...
def _get_deployment_base_url(deployment: dr.Deployment) -> str:
    ...
    if _is_serverless_deployment(deployment):
        ...
        if not pred_server:
            raise ValueError(
                f"Deployment {deployment.id} has no default prediction server"
            )
...
def _get_additional_prediction_instructions(deployment_id: str) -> str:
    """Generate additional instructions for scoring prediction models, to make tool usage more reliable."""
    return f"""
    
Follow these steps in order:
1. Get deployment info: Call tools with the deployment_id="{deployment_id}" to learn about features and requirements.
2. Retrieve features: Use `get_deployment_features` to see all required and optional features with their importance scores.
3. Prepare data: Use `generate_prediction_data_template` to create the correctly structured CSV format.
4. Consider feature importance: For high-importance features, always provide values (infer or ask). Low-importance features can be left blank.
5. Validate: Run `validate_prediction_data` before submission to catch errors early.
6. Time series note: Ensure `datetime_column` and `series_id_columns` are properly formatted if applicable.
